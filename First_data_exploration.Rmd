...
fishdata <- read_sheet("https://docs.google.com/spreadsheets/d/1cbmYDfIc2dxHJxBaowojUZZkN31NW4sL_pHw0t9eTTU/edit#gid=477880397", range = "Data_extraction_2", skip=1, col_types = "ccnccccncncccccnncccnccnncncnccnnncncncccccccc") #with specifying column types
dim(fishdata)
#610  46
names(fishdata)
#View(fishdata)
...
unique(fishdata$Study_ID)

unique(fishdata$Country_firstAuthor)

unique(fishdata$PFAS_type)

unique(fishdata$PFAS_carbon_chain)

unique(fishdata$Cooking_method)

unique(fishdata$Cooking_Category)

unique(fishdata$Oil_type)

unique(fishdata$Unit_PFAS_conc)

unique(fishdata$Sc_technical_biological)

unique(fishdata$Se_technical_biological)

unique(fishdata$Unit_LOD_LOQ)

unique(fishdata$Raw_data_provided)
...
write.csv(fishdata_3, "pilot_data_preprocessed.csv", row.names = F)

fishdata_3 <- read.csv("pilot_data_preprocessed.csv")
```
