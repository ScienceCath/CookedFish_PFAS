...
dat <- read.csv(here("data", "Rawdata.csv"))
str(dat)
...
```{r save final tree}
write.tree(tree2, file="data/phylogenetic_tree.tre") #save the tree

# *NOTE:* underscores within species names on tree tip labels are added automatically
# tree <- read.tree(file="R/plot_cooked_fish_MA.tre") #if you need to read in the tree
...
```{r load tree}
tree <- read.tree(file="data/phylogenetic_tree.tre") #if you need to read in the tree
tree$tip.label <- gsub("_"," ", tree$tip.label) #get rid of the underscores
tree$node.label <- NULL #you can delete internal node labels
#*NOTE:* underscores within species names on tree tip labels are added automatically
```
...
#or plot to pdf
pdf("fig/phylogenetic_tree.pdf", width=8, heigh=16)
