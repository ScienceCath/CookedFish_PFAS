...
# TODO - Patrice this is a new mode
# Volume_liquid_ml_0
# the issue here is 0 cannot be log-transformed so adding 1

volume2_model<-run_model(dat, ~scale(log(Volume_liquid_ml_0+1))) # logged and z-transformed after adding 1
  
summary(volume2_model)
r2_ml(volume2_model)

# Plot
dat.volume2<-filter(dat, Volume_liquid_ml_0!="NA")
# TODO
# make 0 = no liquid visible???
plot_continuous(dat.volume2, volume2_model, log(dat.volume2$Volume_liquid_ml), "Volume of liquid (mL)")
...
#TODO - full models need updating 
# use Volume_liquid_ml_0????
...
#TODO -- added
dat$log1_Volume_liquid_ml<-log(dat$Volume_liquid_ml_0 + 1)

full_model_org_units1 <- run_model(dat, ~ - 1 + 
                               Cooking_Category +
                               Temperature_in_Celsius +
                               Length_cooking_time_in_s +
                               PFAS_carbon_chain +
                               log1_Volume_liquid_ml)
...
# TODO- added
full_model_org_units_oil_water1<- run_model(dat_oil_water, ~ - 1 + 
                               Cooking_Category +
                               Temperature_in_Celsius +
                               Length_cooking_time_in_s +
                               PFAS_carbon_chain +
...
res1<-marginal_means(model = full_model_org_units1, data=dat, mod="1")
res1$mod_table
```
...
#TODO - added
res_cat1<-marginal_means(full_model_org_units1, data = dat, mod = "1", by = "Cooking_Category")
res_cat1$mod_table

orchard_plot(res_cat1, xlab = "lnRR", condition.lab = "Cooking Category" )
