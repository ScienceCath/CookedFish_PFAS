...
emmeans(res_mean_values, specs = ~1, df = full_model_org_units$ddf) # Marginalised mean at the mean value of each predictor in the full model
...
emmeans(res_mean_values, specs = ~1, df = full_model_org_units$ddf, weights="prop") # Setting proportional weights returns similar results
```

## Marginalised means for different cooking categories

```{r}
emmeans(res_mean_values, specs = "Cooking_Category", df = full_model_org_units$ddf)
```
...
```{r}
emmeans(res_cooking_time , specs = ~1 |  Length_cooking_time_in_s, df = full_model_org_units$ddf) 
```

### Marginalised mean estimate for each cooking category, at each cooking time

```{r}
emmeans(res_cooking_time , specs = ~ Cooking_Category |  Length_cooking_time_in_s, df = full_model_org_units$ddf) 
```
...
emmeans(res_volume , specs = ~ 1 |  log_Volume_liquid_ml, df = full_model_org_units$ddf) 
```
...
```{r}
emmeans(res_PFAS, specs = ~ 1 |  PFAS_carbon_chain, df = full_model_org_units$ddf) 
```

### Marginalised mean estimate for each PFAS carbon chain, for each cooking category 

```{r}
emmeans(res_PFAS, specs = ~ Cooking_Category |  PFAS_carbon_chain, df = full_model_org_units$ddf) 
```
##
