...
#remotes::install_github("rvlenth/emmeans", dependencies = TRUE, build_opts = "")  # Note that the installation may not work if the folder is synchronizing with OneDrive
...
# averaged 
# TODO - a note for Patrice - there was a msitake in this so all results could change
lnRR_func <- function(Mc, Nc, Me, Ne, aCV2c, aCV2e, rho = 0.5){
  lnRR <- log(Me/Mc) + 
        0.5 * ((aCV2e/Ne) - (aCV2c/Nc))	
  
  var_lnRR <- (aCV2c/Nc) + (aCV2e/Ne) - 
         2*rho*(sqrt(aCV2c)*sqrt(aCV2e)/(Nc)) # there was some mistake - Nc = Ne = always hold
...
#TODO Patice - in the paper - we introduce the result in this order so please rearrange - ) thermal processing or cooking time, 2) cooking liquid volume (or the amount of liquid, used for cooking), 3) cooking temperature, 4) the number of carbon chains in PFAS and 5) cooking categories 
...
```{r, fig.width=10, fig.height=7}
# TODO - this is not written in the manuscript yet - chat with Patrice - may leave just for suppl??
...
VCV<-make_VCV_matrix(dat
                       , V = "var_lnRR", cluster = "Cohort_ID", obs = "Effect_ID", rho = 0.5) # create VCV matrix

# testing cooking categories  
full_modeld  <- rma.mv(yi = lnRR, V = VCV, # run the model, as described earlier
                         mods= ~-1 +
                               Cooking_Category +
                               scale(Temperature_in_Celsius) +
                               scale(Length_cooking_time_in_s) +
         ...
         test = "t", 
         data = dat,
         btt = c(1:3)) # testing the significance of cooking category - testing first 3 regression coefficents) 
...
emmeans(res_mean_values, specs = ~1, df = full_model_org_units$df) # Marginalised mean at the mean value of each predictor in the full model
# When all predictors at set to their mean level, the mean effect size is -0.556 (42.7% decrease in PFAS content in experimental vs. control samples)

emmeans(res_mean_values, specs = ~1, df = full_model_org_units$df, weights="prop") # Setting proportional weights returns similar results

# TODO
overall <-marginal_means(full_model_org_units, data = dat, mod = "1", at = list(Length_cooking_time_in_s = c(120,600,1500)), by = "Length_cooking_time_in_s")
 
orchard_plot(overall, xlab = "lnRR", condition.lab = "Cooking time (sec)")
# probably use log10???
overall2 <-marginal_means(full_model_org_units, data = dat, mod = "1", at = list(log_Volume_liquid_ml= c(2.3, 5.5, 9.2)), by = "log_Volume_liquid_ml")
 
orchard_plot(overall2, xlab = "lnRR", condition.lab = "ln(liquid volume (ml))")
```

## Marginal means for different cooking categories

```{r}
emmeans(res_mean_values, specs = "Cooking_Category", df = full_model_org_units$df)
```
...
```{r}
res_cooking_time <- marginal_means(full_model_org_units, data = dat, mod = "Cooking_Category", at = list(Length_cooking_time_in_s = c(120,600,1500)), by = "Length_cooking_time_in_s")
 
orchard_plot(res_cooking_time, xlab = "lnRR", condition.lab  = "Cooking time (sec)")

# this is less meaningfull - as no liquid goup has just 0 liquid
#test <-marginal_means(full_model_org_units, data = dat, mod = "Cooking_Category", at = list(log_Volume_liquid_ml= c(2.3, 5.5, 9.2)), by = "log_Volume_liquid_ml")
 
#orchard_plot(test, xlab = "lnRR", condition.lab  = "log(liquid volume) (ml)")


```

### Marginal mean estimate at each cooking time

```{r}
emmeans(res_cooking_time , specs = ~1 |  Length_cooking_time_in_s, df = full_model_org_units$df) 
```

### Marginal mean estimate for each cooking category, at each cooking time

```{r}
emmeans(res_cooking_time , specs = ~ Cooking_Category |  Length_cooking_time_in_s, df = full_model_org_units$df) 
```
...
emmeans(res_volume , specs = ~ 1 |  log_Volume_liquid_ml, df = full_model_org_units$df) 
```

## Marginal means for different PFAS carbon chains

Here, we generate marginalized estimates for PFAS of 3, 6, 9 and 12 carbon chains
...
```{r}
emmeans(res_PFAS, specs = ~ 1 |  PFAS_carbon_chain, df = full_model_org_units$df) 
```

### Marginalised mean estimate for each PFAS carbon chain, for each cooking category 

```{r}
emmeans(res_PFAS, specs = ~ Cooking_Category |  PFAS_carbon_chain, df = full_model_org_units$df) 
```
##
...
#TODO - probalby remove Temp there is no varation here
full_model_water <- run_model_water(water_dat, ~ scale(Temperature_in_Celsius) + # we said we did not test in our method - as there is no variation in this moderator
                                           scale(Length_cooking_time_in_s) +
                                           scale(PFAS_carbon_chain) +
                                           scale(log(Volume_liquid_ml)))
...
# TODO use this model to create plots for pubication bias as well as time lag
...
#TODO - do this for the full model (probably not with year or effective sample size)
...
p_time<-ggplot(dat,aes(x = Length_cooking_time_in_s, y = lnRR)) +
       ...
       geom_ribbon(data=pred_full_model_time, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_full_model_time,aes(y = pred), size = 1.5, color="black")+  
  
       geom_ribbon(data=pred_uni_model_time, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_uni_model_time,aes(y = pred), size = 1.5, col="black", lty="dashed")+  
...
p_vol<-ggplot(dat,aes(x = log_Volume_liquid_ml, y = lnRR)) +
       ...
       geom_ribbon(data=pred_full_model_vol, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_full_model_vol,aes(y = pred), size = 1.5, color="black")+  
  
       geom_ribbon(data=pred_uni_model_vol, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_uni_model_vol,aes(y = pred), size = 1.5, col="black", lty="dashed")+  
...
p_temp<-ggplot(dat,aes(x = Temperature_in_Celsius, y = lnRR)) +
       ...
       geom_ribbon(data=pred_full_model_temp, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_full_model_temp,aes(y = pred), size = 1.5, color="black")+  
  
       geom_ribbon(data=pred_uni_model_temp, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_uni_model_temp,aes(y = pred), size = 1.5, col="black", lty="dashed")+  
...
p_PFAS<-ggplot(dat,aes(x = PFAS_carbon_chain, y = lnRR)) +
       ...
       geom_ribbon(data=pred_full_model_PFAS, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_full_model_PFAS,aes(y = pred), size = 1.5, color="black")+  
  
       geom_ribbon(data=pred_uni_model_PFAS, aes(ymin = ci.lb, ymax = ci.ub, color=NULL), alpha = 0.2) +
       geom_line(data=pred_uni_model_PFAS,aes(y = pred), size = 1.5, col="black", lty="dashed")+  
