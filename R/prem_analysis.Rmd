    ...
    if (names(mod_table)[2] == "condition") {
        ...
        plot <- ggplot2::ggplot() + ggbeeswarm::geom_quasirandom(data = data, 
            ...
            ggplot2::geom_linerange(data = mod_table, ggplot2::aes(x = name, 
                ymin = lowerPR, ymax = upperPR), size = 0.75, # change size confidence intervals and swap CL with PR
            ...
            ggplot2::geom_pointrange(data = mod_table, ggplot2::aes(y = estimate, 
                ...
                fill = name), size = 2, stroke=1.2,position = ggplot2::position_dodge2(width = 0.3)) + # change size point and prediction intervals
...
p_time_mm<-my_orchard(time_mm, xlab = "lnRR", condition.lab = "Cooking time (sec)", alpha=0.4)+
           scale_size_continuous(range = c(1, 10))+
...
p_volume_mm<-my_orchard(volume_mm, xlab = "lnRR", condition.lab = "ln(liquid volume (ml))", alpha=0.4)+
           scale_size_continuous(range = c(1, 10))+
           scale_fill_manual(values="deepskyblue2")+
           scale_colour_manual(values = "deepskyblue3")+ # change colours
           theme(panel.border = element_rect(colour = "black", fill=NA, size=1.3), # border around the plot
                 text = element_text(size = 24), # change font sizes
                 legend.title = element_text(size = 13),
                 legend.text = element_text(size = 10),
...
p_time_mm_cat<-my_orchard(time_mm_cat, xlab = "lnRR", condition.lab  = "Cooking time (sec)", alpha=0.4)+
           scale_size_continuous(range = c(1, 10))+
           scale_fill_manual(values=c("#55C667FF", "goldenrod2", "dodgerblue3"))+
           scale_colour_manual(values = c("#55C667FF", "goldenrod2", "dodgerblue3"))+ # change colours
           theme(panel.border = element_rect(colour = "black", fill=NA, size=1.3), # border around the plot
...
```{r, fig.width=9, fig.height=7}
dat$Publication_year=as.integer(dat$Publication_year)
...
p_5C<-ggplot(dat,aes(x = Publication_year, y = lnRR)) +
       ...
       labs(x = "Publication_year", y = "lnRR", size = "Precison (1/SE)") + 
  scale_size_continuous(range=c(1,10))+
  theme_bw() +
  geom_hline(yintercept = 0,linetype = 2, colour = "black",alpha=0.5)+   # horizontal line at lnRR = 0
  theme(text = element_text(size = 18, colour = "black", hjust = 0.5), # change font sizes and legend position
...
png("fig/Fig_5A.png", height=8, width=7, units="in", res=1200)
plot(funell)
dev.off()
```
...
ggsave("fig/Fig_5BC.png", width=15, height=12, dpi=1200)
```
