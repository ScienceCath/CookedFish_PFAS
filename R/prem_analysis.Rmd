...
### Ratio between tissue sample and liquid volume
...
### Ratio between tissue sample and liquid volume, with no-liquid coded as "0"

```{r, fig.width=10, fig.height=7}
# Ratio_liquid_fish with "0" for no-liquid
...
save(full_model_org_units, full_model_org_units0, full_model_org_units_oil_water, file = here("Rdata", "full_models_org_units.RData"))
```
...
### Overall mean for each cooking category, at the mean of each other predictor, with Ratio_liquid_fish taken as `NA` for the dry cooking category
...
### Overall mean for each cooking category, at the mean of each other predictor, with Ratio_liquid_fish taken as `0` for the dry cooking category

```{r, fig.width=10, fig.height=7}
res_cat0<-marginal_means(full_model_org_units0, data = dat, mod = "1", by = "Cooking_Category")
res_cat0$mod_table

orchard_plot(res_cat0, xlab = "lnRR", condition.lab = "Cooking Category" )
```
...
### Marginalised means for each cooking category, at different cooking times, with Ratio_liquid_fish taken as `NA` for the dry cooking category
...
```{r, fig.width=10, fig.height=7}
res_cooking_time_cat0 <-marginal_means(full_model_org_units0, data = dat, mod = "Cooking_Category", at = list(Length_cooking_time_in_s = c(120,600,1500)), by = "Length_cooking_time_in_s")
res_cooking_time_cat0$mod_table

orchard_plot(res_cooking_time_cat0, xlab = "lnRR", condition.lab = "Cooking time (sec)")
...
orchard_plot(res_volume0, xlab = "lnRR", condition.lab = "ln(Liquid volume to tissue sample ratio + 1 (mL/g))")
```
...
### Marginalised mean estimate for each PFAS carbon chain, for each cooking category, with Ratio_liquid_fish taken as `NA` for the dry cooking category
...
### Marginalised mean estimate for each PFAS carbon chain, for each cooking category, with Ratio_liquid_fish taken as `0` for the dry cooking category

```{r, fig.width=10, fig.height=7}
res_PFAS_cat0<-marginal_means(full_model_org_units0, data = dat, mod = "Cooking_Category", at = list(PFAS_carbon_chain= c(3, 6, 12)), by = "PFAS_carbon_chain")
res_PFAS_cat0$mod_table

orchard_plot(res_PFAS_cat0, xlab = "lnRR", condition.lab = "PFAS carbon chain")
```
...
```{r}
full_model_oil0 <- run_model_oil(oil_dat, ~ scale(Temperature_in_Celsius) +
                                           scale(Length_cooking_time_in_s) +
                                           scale(PFAS_carbon_chain) +
                                           scale(log(Ratio_liquid_fish_0 + 1)))
...
```{r}
full_model_water0 <- run_model_water(water_dat, ~
                                           scale(Length_cooking_time_in_s) +
                                           scale(PFAS_carbon_chain) +
                                           scale(log(Ratio_liquid_fish_0 + 1)))
...
((p_time_mm0/p_volume_mm0)|p_time_mm_cat0) + plot_annotation(tag_levels=c("A", "B", "C"))
```
...
```{r, fig.height=8, fig.width=10}
##### Oil based
full_model_oil_vol0<- run_model_oil(oil_dat, ~ scale(Temperature_in_Celsius) +
                                           scale(Length_cooking_time_in_s)+
                                           scale(PFAS_carbon_chain) +
                                           log_Ratio_liquid_fish0)
